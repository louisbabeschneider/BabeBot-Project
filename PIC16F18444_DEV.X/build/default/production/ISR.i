...
_Bool flag_CMD;
_Bool flag_Reboot;
_Bool flag_Connected;
_Bool flag_Connection_End;
_Bool flag_send_status_required;
_Bool flag_END;
_Bool flag_desired_Bbot_status;
...
void __attribute__((picinterrupt(("")))) INTERRUPT_InterruptManager (void){
    ...
    if(INTCONbits.PEIE == 1)
    {
        ...
        if (buff_RX[index_RX] == 10 || buff_RX[index_RX] ==58){
            if (strncmp(buff_RX, "AOK", index_RX-1) == 0 && index_RX > 2) flag_AOK = 1;
            if (strncmp(buff_RX, "CMD", index_RX-1) == 0 && index_RX > 2) flag_CMD = 1;
            if (strncmp(buff_RX, "ConnParam", index_RX-1) == 0 && index_RX > 6 ) flag_Connected = 1;
            if (strncmp(buff_RX, "Reboot", index_RX-1) == 0 && index_RX > 2) flag_Reboot = 1;
            if (strncmp(buff_RX, "Connection End", index_RX-1 ) == 0 && index_RX > 6 ) {flag_Connection_End = 1;}
            if (strncmp(buff_RX, "ERR", index_RX-1) == 0 && index_RX > 2) flag_ERR = 1;
            if (strncmp(buff_RX, "END", index_RX-1) == 0 && index_RX > 2) flag_END = 1;
            if (strncmp(buff_RX, "R,0", index_RX-3) == 0 && index_RX > 2){
                if (buff_RX[index_RX-3] == 48){
                    flag_desired_Bbot_status = 1;
        ...
        else if (index_RX >= 20 -1 )index_RX=0;
        else index_RX++;
        }
    }
